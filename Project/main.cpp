...
void menu(Library& library) {
  string options = "1. Add default books to the library\n"
                   ...
                   "8. Search by author\n"
                   "9. Search by genre\n"
                   "10. Sort the books by page count\n"
                   "11. Sort the books by publication year\n"
                   "12. Exit";
  int option = 0;
    ...
    switch (option) {
      ...
      case 10: { // Page count
        ...
        for(int i = 0; i < size; i++) {
          Book& currentBook = library.getBooks()[i];
          pages[i] = currentBook.getPages();
        }
        library.clear();
        library.mergeSort(pages, size);
        for(int i = 0; i < size; i++) {
          Book* currentBook = library.searchByPages(pages[i]);
          library.addBook(*currentBook);
        }
        break;
      }
      case 11: { // Publication year
        int size = library.size();
        int years[size];
        for(int i = 0; i < size; i++) {
          Book& currentBook = library.getBooks()[i];
          years[i] = currentBook.getYear();
        }
        library.clear();
        library.mergeSort(years, size);
        for(int i = 0; i < size; i++) {
          Book* currentBook = library.searchByYear(years[i]);
          library.addBook(*currentBook);
        }
        break;
      }
